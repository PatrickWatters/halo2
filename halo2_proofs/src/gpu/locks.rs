...
use ec_gpu_gen::program::program;
...
/*
fn create_fft_kernel2<'a, F>(priority: bool) -> Option<(FftKernel<'a, F>, GPULock<'a>)>
where
    F: Field + GpuName,
{
    ...
    E: Engine + GpuEngine,
{
    ...
    let kernel = if priority {
        FftKernel::<halo2curves::bn256::Bn256>::create(&programs);
    } else {
        // Low priority kernels may be aborted if a high priority kernel wants to run/is running.
        FftKernel::<halo2curves::bn256::Bn256>::create_with_abort(&programs,&PriorityLock::is_taken);
    };
    match kernel {
        Ok(k) => {
            info!("GPU FFT kernel instantiated!");
    ...
    //let mut kernel = FftKernel::<halo2curves::bn256::Bn256>::create(&devices).expect("Cannot initialize kernel!");
    
}

fn create_multiexp_kernel<'a, E>(
    priority: bool,
) -> Option<(MultiexpKernel<'a, E>, GPULock<'a>)>
where
    E: Engine + GpuEngine,
{
    let lock = GPULock::lock();
    ...
    let kernel = if priority {
        MultiexpKernel::<halo2curves::bn256::Bn256>::create(&devices);
    } else {
        // Low priority kernels may be aborted if a high priority kernel wants to run/is running.
        MultiexpKernel::<halo2curves::bn256::Bn256>::create_with_abort(&devices,&PriorityLock::is_taken);
    };
...
fn create_multiexp_kernel2<'a, G>(
    priority: bool,
...
locked_kernel!(
    ...
    create_fft_kernel,
    "FFT",
    pub struct LockedFftKernel<'a, E> where E: Engine + GpuEngine,
);
locked_kernel!(
    MultiexpKernel<'a, E>,
